...
export function PostPreview({ platform, caption, seed, brand = "yourbrand", imageUrl }: { platform: Platform; caption: string; seed: string; brand?: string; imageUrl?: string | null }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border/60 bg-card">
      ...
      <div className={`relative w-full overflow-hidden ${aspect[platform]} bg-background/40`}>
        {imageUrl ? <img src={imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover" /> : <PostArt seed={seed} className="absolute inset-0 h-full w-full" />}
